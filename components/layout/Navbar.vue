...
const navbarHeightStyle = computed(() => {
  const heights = getPageLayout.value?.navbar?.height;
  if (!heights) {
...
const logoHeightStyle = computed(() => {
  const heights = navbarLogo.value?.height;
  if (!heights) {
    return { height: isMobile.value ? '24px' : isTablet.value ? '28px' : '28px' };
  }
  return {
    height: isMobile.value ? heights.mobile : isTablet.value ? heights.tablet : heights.desktop
  };
});
